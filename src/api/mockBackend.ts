// 模拟后端API接口
import type { Dish, Category } from '../stores/menu';
import type { Meal } from '../stores/order';

// 模拟数据存储
let mockCategories: Category[] = [];
let mockDishes: Dish[] = [];
let mockOrders: Record<string, Record<string, number>> = {}; // date::meal -> {dishId: count}

// 初始化模拟数据
function initMockData() {
  if (mockCategories.length > 0) return;
...
export const mockAPI = {
  ...
  async getDishes(): Promise<Dish[]> {
    initMockData();
    return new Promise(resolve => {
      setTimeout(() => resolve([...mockDishes]), 300);
    });
  },
  
  // 获取指定日期和餐次的订单
  async getOrder(date: string, meal: Meal): Promise<Record<string, number>> {
    initMockData();
    const key = `${date}::${meal}`;
    return new Promise(resolve => {
      setTimeout(() => resolve(mockOrders[key] || {}), 300);
    });
  },
  
  // 提交订单
  async submitOrder(date: string, meal: Meal, dishes: Record<string, number>): Promise<boolean> {
    initMockData();
    const key = `${date}::${meal}`;
    mockOrders[key] = { ...dishes };
    return new Promise(resolve => {
  ...
  // 获取菜品详情
  async getDishDetail(dishId: string): Promise<Dish | undefined> {
    initMockData();
    return new Promise(resolve => {
      setTimeout(() => {
        const dish = mockDishes.find(d => d.id === dishId);
        resolve(dish ? { ...dish } : undefined);
      }, 300);
    });
  }
};